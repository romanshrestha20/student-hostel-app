// roomController.js (Refactored with improvements and JSDoc)
import prisma from "../prisma/client.js";
import {
  validateRoomData,
  validateUpdateRoomData,
} from "../middlewares/validiators.js";
...
export const getAllRooms = async (req, res) => {
  try {
    ...
    console.error("[RoomController] Error fetching rooms:", error);
    res.status(500).json({ error: "Failed to fetch rooms" });
  }
};

/**
 * GET /rooms/type?roomType=TYPE - Get rooms by room type
 */
export const getRoomsByRoomType = async (req, res) => {
  try {
    ...
    if (rooms.length === 0) {
      return res
        .status(404)
        .json({ error: "No rooms found for this room type" });
    }

    res.status(200).json({ message: "Rooms fetched successfully", rooms });
  } catch (error) {
    console.error("[RoomController] Error fetching rooms by type:", error);
...
export const getAllRoomsByHostel = async (req, res) => {
  try {
    ...
    console.error("[RoomController] Error fetching rooms by hostel:", error);
    res.status(500).json({ error: "Failed to fetch rooms" });
  }
};
...
export const getRoomById = async (req, res) => {
  try {
    ...
    console.error("[RoomController] Error fetching room by ID:", error);
  ...
  try {
    ...
    if (!validateRoomData(req.body)) {
      return res.status(400).json({
        error:
          "Missing or invalid required fields: hostelId, roomType, price, capacity",
      });
    }
